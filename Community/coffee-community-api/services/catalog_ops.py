...
def run_scrape_job(
    ...
    *,
    ...
    Always routes through `run_enrichment_v2_job` as of 2026-05-25
    (v1 retirement). The legacy subprocess + stage_proposals body
    below this delegation is dead code and will be removed in the
    follow-up cleanup pass; it remains here only because the file's
    scrape_runner module imports + helper functions are non-trivial
    to disentangle in one edit.
    """
    slugs = [roaster_slug] if roaster_slug else None
    return run_enrichment_v2_job(
        job_id,
        roaster_slugs=slugs,
        kinds=("product",),
        force_enrich=False,
        regenerate_hint=regenerate_prompt,
    )
...
def scrape_one_roaster(
    *,
    ...
    parent_operation_id: when set, the enrichment op chains under
    the given parent (e.g. full_reenrich_roaster) so the audit trail
    threads correctly.
    """
    db = get_db()
    try:
        ...
        db.close()
    run_enrichment_v2_job(
        job_id,
        roaster_slugs=[roaster_slug],
        kinds=("product",),
        force_enrich=force_enrich,
        regenerate_hint=regenerate_prompt,
        parent_operation_id=parent_operation_id,
    )
    # Re-read the row so callers downstream of the legacy
    # contract get the same dict shape (result_summary).
    db = get_db()
    try:
        row = db.execute(
            "SELECT result_summary FROM jobs WHERE id = ?",
            (job_id,),
        ).fetchone()
        import json as _json
        return _json.loads(row["result_summary"]) if row and row["result_summary"] else {}
    finally:
        db.close()
...
def set_product_available(
    db, product_id: str, available: bool, *, started_by: int
) -> dict:
    ...
    The companion to mark_product_sold_out: that tool can only HIDE a
    bean (available=0); this one can also UN-HIDE one (available=1)
    without a full re-enrich. Built 2026-05-30 after a thin Magento
    fetch was suspected of hiding an in-stock bean and the only
    recovery path was a full re-enrich. Uses the modern
    catalog_operations audit path (not the legacy jobs/proposals path
    mark_product_sold_out uses) so rollback restores the exact prior
    `available` value via the captured snapshot.
    """
    from services import operation_qc

    target = 1 if available else 0
    row = db.execute(
        f"SELECT {scrape_runner.PRODUCT_LITE_COLS} FROM products "
        "WHERE product_id = ?",
        (product_id,),
    ).fetchone()
    if not row:
        ...
        raise HTTPException(404, f"Product {product_id} not found")
    prev = scrape_runner._product_lite_from_row(row)
    prev_available = int(prev["available"]) if prev["available"] is not None else None
    if prev_available == target:
        return {
            "product_id": product_id,
            "available": target,
            "noop": True,
            "note": f"already available={target}",
        }

    op_id = operation_qc.start_operation(
        db,
        kind="manual_set_available",
        target_slug=prev.get("roaster_slug"),
        params={
            "product_id": product_id,
    ...
    # Snapshot the full pre-mutation row so rollback restores the exact
    # prior `available` value (mutation_kind='update' → rollback
    # overwrites with the captured before-state).
    operation_qc.snapshot_rows(
        db, op_id, "products", [dict(row)], mutation_kind="update",
    )
    db.execute(
...
def run_standardize_job(job_id: int, *, regenerate_exemplars: bool = True,
    ...
    `regenerate_exemplars` defaults to True (2026-05-25 change): the
    user wants exemplars refreshed every standardize run so the
    classifier stays primed with the latest house-style examples.
    Caller can pass `regenerate_exemplars=False` to reuse cached
    exemplars (e.g. for tight admin debug loops).
    """
    db = get_db()
    log_lines: list[str] = []
    ...
    # Op-QC wrapper. The op_duplicate_enqueue_burst T1 rule needs
    # this row to count prior standardize ops within the 5-min
    # window; the lock+follow-up regression detector lives here.
    ...
    op_id = start_operation(
        db, kind="standardize",
        params={
            "regenerate_exemplars": regenerate_exemplars,
            "tasks": tasks, "force_reclassify": force_reclassify,
            "job_id": job_id,
        },
    )
...
def run_roaster_enrich_job(job_id: int, *, website: str,
    ...
    """
    db = get_db()
    # Op-QC wrapper. Onboarding is a high-impact op (creates new
    # roaster_profiles + roaster_sources + first product set).
    from services.operation_qc import (
        start_operation, finish_operation_with_qc, finish_operation,
    )
    op_id = start_operation(
        db, kind="onboard_roaster",
        params={"website": website, "also_scrape": also_scrape,
                "job_id": job_id},
    )
    try:
        mark_running(db, job_id)
        ...
        # Update op_kind target_slug now that we know the resolved slug
        try:
            db.execute(
                "UPDATE catalog_operations SET target_slug = ? WHERE id = ?",
                (applied.get("slug"), op_id),
            )
            db.commit()
            finish_operation_with_qc(
                db, op_id, status="succeeded",
                summary={
                    **result_summary,
                    "rows_inserted": 1,  # the roaster_profile row
                },
            )
        except Exception:
            pass
...
def run_article_scrape_job(job_id: int, *,
                              ...
                              force_enrich: bool = False,
                              regenerate_article_hint: bool = False,
                              parent_operation_id: Optional[int] = None) -> None:
    ...
    Always routes through `run_enrichment_v2_job` as of 2026-05-25
    (v1 retirement). The legacy in-process article scraper body
    below this delegation is dead code and will be removed in the
    follow-up cleanup pass.

    Per-source steps (v1 reference — v2 has its own pipeline):
    ...
    """
    # Honor both single-slug and multi-slug entry points by
    # collapsing into a list (None preserved for "all enabled").
    slugs = roaster_slugs if roaster_slugs else (
        [roaster_slug] if roaster_slug else None
    )
    return run_enrichment_v2_job(
        job_id,
        roaster_slugs=slugs,
        kinds=("article",),
        force_enrich=force_enrich,
        regenerate_hint=regenerate_article_hint,
        parent_operation_id=parent_operation_id,
    )
    # Re-stamp the pipeline contextvar — threading.Thread doesn't
...
# ── V2 enrichment runner (single function for both kinds) ──────────────────


def run_enrichment_v2_job(
    job_id: int,
    *,
    roaster_slugs: Optional[list[str]] = None,
    kinds: tuple[str, ...] = ("product", "article"),
    force_enrich: bool = False,
    regenerate_hint: bool = False,
    parent_operation_id: Optional[int] = None,
) -> None:
    """V2 BG-task entry. Replaces the subprocess + stage_proposals path
    of `run_scrape_job` and the per-roaster loop of
    `run_article_scrape_job` with one unified runner that calls
    `enrichment_runner.run_for_roaster` for every roaster x kind.

    `roaster_slugs=None` means "all published roasters". `kinds`
    restricts to one or the other if needed.

    Operator-agnostic per AGENTIC_UTOPIA.md rule 1 — the underlying
    `entity_enricher.enrich_url` routes through `llm_router.call_llm`
    which picks queue (agent) or SDK (human) automatically.
    """
    ...
    aggregate = {
        ...
        "per_kind": {
            k: {
                ...
                "skipped_already_enriched": 0, "failed": 0,
                "gate_rejected": 0,
            }
            for k in kinds
        },
        "site_hints": {
            ...
            "cached": 0, "no_pattern": 0, "failed": 0,
        },
        "errors": [],
        "per_roaster": [],
        "cancelled": False,
    ...
    # Operation-QC wrapper (2026-05-27). Logs start/finish into
    # catalog_operations so the orchestrator can audit + roll back
    # via the new MCP surface. parent_operation_id chains this run
    # under a containing op (e.g. full_reenrich_roaster) when set.
    from services.operation_qc import (
        start_operation, finish_operation_with_qc, finish_operation,
    )
    op_kind = "enrich_all" if not roaster_slugs else (
        "scrape_one_roaster" if len(roaster_slugs) == 1 and kinds == ("product",)
        else "enrich_per_slug"
    )
    op_id = start_operation(
        db, kind=op_kind,
        target_slug=roaster_slugs[0] if roaster_slugs and len(roaster_slugs) == 1 else None,
        params={
            "roaster_slugs": roaster_slugs, "kinds": list(kinds),
            "force_enrich": force_enrich,
    ...
    try:
        mark_running(db, job_id)
        log(
            f"v2 enrichment starting (kinds={list(kinds)}, "
            f"scope={'all' if not roaster_slugs else len(roaster_slugs)} roaster(s), "
            f"force_enrich={force_enrich}, regenerate_hint={regenerate_hint})"
        )
        ...
        if roaster_slugs:
            ...
            rows = db.execute(
                ...
                f"WHERE roaster_slug IN ({placeholders}) AND published = 1 "
                f"ORDER BY roaster_slug",
                roaster_slugs,
            ).fetchall()
        else:
            rows = db.execute(
                "SELECT roaster_slug FROM roaster_profiles "
                "WHERE published = 1 ORDER BY roaster_slug"
            ).fetchall()

        slugs = [r["roaster_slug"] for r in rows]
        log(f"iterating {len(slugs)} roaster(s)")
        if not slugs:
            mark_finished(
                db, job_id, status="succeeded",
                log_tail="\n".join(log_lines + ["no roasters to enrich"]),
        ...
        for slug in slugs:
            ...
            log(f"--- {slug} ---")
            try:
                rr = enrichment_runner.run_for_roaster(
                    db, slug, kinds=kinds,
                    force_enrich=force_enrich,
                    regenerate_hint=regenerate_hint,
                    job_id=job_id,
                ...
                aggregate["roasters_failed"] += 1
                aggregate["errors"].append({"slug": slug, "message": f"{type(e).__name__}: {e}"})
                log(f"  FAILED: {type(e).__name__}: {e}")
                continue

            aggregate["roasters_processed"] += 1
            aggregate["per_roaster"].append(rr.to_summary())

            # Stamp freshness timestamps so observability tools
            # (crema_freshness_report, crema_diff_sweep, the admin
            # UI's freshness dashboard) see this roaster as
            # recently-scraped. Without this, agents using
            # `has_diff=true` or "older than N days" filters get
            # stale results post-v2 run. Mirrors v1 path.
            try:
                if "product" in kinds:
                    scrape_runner.stamp_sources_scraped(db, roaster_slug=slug)
                if "article" in kinds:
                    db.execute(
                        "UPDATE roaster_sources SET last_articles_scraped_at = ? "
                        "WHERE LOWER(website) IN (SELECT LOWER(website) "
                        "FROM roaster_profiles WHERE roaster_slug = ?)",
                        (_now(), slug),
                    )
                    db.commit()
            except Exception as e:
                log(f"  warn: failed to stamp freshness for {slug}: {e}")
            for k, s in rr.per_kind.items():
                bucket = aggregate["per_kind"].setdefault(k, {
                    "discovered": 0, "enriched": 0, "inserted": 0,
                    "updated": 0, "skipped_unchanged": 0,
                    "skipped_already_enriched": 0, "failed": 0,
                    "gate_rejected": 0,
                })
                bucket["discovered"] += s.discovered
                bucket["enriched"] += s.enriched
                bucket["inserted"] += s.inserted
                bucket["updated"] += s.updated
                bucket["skipped_unchanged"] += s.skipped_unchanged
                bucket["skipped_already_enriched"] += s.skipped_already_enriched
                bucket["failed"] += s.failed
                bucket["gate_rejected"] += s.gate_rejected
            for kind, hint_state in rr.site_hint_status.items():
                if hint_state in aggregate["site_hints"]:
                    aggregate["site_hints"][hint_state] += 1
            for err in rr.errors:
                aggregate["errors"].append({"slug": slug, "message": err})

        _flush()
        # Emit v1-compatible top-level keys so the existing admin UI
        # parser (`summarizeJob` in JobHistory.tsx) renders cleanly
        # without a frontend change. v2-native keys (per_kind,
        # site_hints, per_roaster) sit alongside for v2-aware readers.
        prod = aggregate["per_kind"].get("product", {})
        art = aggregate["per_kind"].get("article", {})
        aggregate.update({
            # v1 product keys
            "scraped": prod.get("enriched", 0) + prod.get("skipped_already_enriched", 0),
            "new_products_total": prod.get("inserted", 0),
            "updated_total": prod.get("updated", 0),
            "missing_total": 0,
            "restoring_total": 0,
            "skipped": prod.get("gate_rejected", 0) + prod.get("skipped_unchanged", 0),
            "enrichment_failures": prod.get("failed", 0),
            # v1 article keys
            "articles_inserted": art.get("inserted", 0),
            "articles_updated": art.get("updated", 0),
            "articles_skipped": (
                art.get("gate_rejected", 0)
                + art.get("skipped_unchanged", 0)
                + art.get("skipped_already_enriched", 0)
            ),
            "discoveries": art.get("discovered", 0),
            "enriched": art.get("enriched", 0),
            "enrich_failed": art.get("failed", 0),
            "not_article_skipped": 0,
            "off_topic_skipped": 0,
            "empty_skipped": 0,
            "hints_generated": aggregate["site_hints"]["generated"],
            "hints_regenerated": aggregate["site_hints"]["regenerated"],
            "hints_failed": aggregate["site_hints"]["failed"],
        })
        mark_finished(
            db, job_id, status="succeeded",
            log_tail="\n".join(log_lines)[-50_000:],
        ...
        # Op-QC summary: surface the key counts T1 rules consume
        # (enriched_total, products_discovered, failed). Reuse the
        # aggregate dict + add a top-level enriched_total convenience.
        op_summary = {
            **{k: aggregate[k] for k in (
                "roasters_processed", "roasters_failed", "cancelled",
                "scraped", "new_products_total", "updated_total",
                "enrichment_failures", "discoveries",
            ) if k in aggregate},
            "kinds": list(kinds),
            "enriched_total": prod.get("enriched", 0) + art.get("enriched", 0),
            "products_discovered": prod.get("discovered", 0),
            "failed": prod.get("failed", 0) + art.get("failed", 0),
            "rows_kept": prod.get("inserted", 0) + prod.get("updated", 0)
                          + art.get("inserted", 0) + art.get("updated", 0),
        }
        try:
            finish_operation_with_qc(
                db, op_id, status="succeeded", summary=op_summary,
            )
        except Exception:
            pass
    except Exception as e:
        ...
        try:
            finish_operation_with_qc(
                db, op_id, status="failed", summary=aggregate,
                error_message=f"{type(e).__name__}: {e}",
            )
        except Exception:
            pass
