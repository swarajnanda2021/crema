...
Captured from the live Expo Go run at the end of the session. The
preview-browser screenshots look correct, so the regressions below
are either native-only or edge-case paths that didn't surface on
web. Next session: reproduce each on device first, then patch.

| # | Bug | Notes |
|---|-----|-------|
| M1 | **Native "simply does not work" after §postmodal-redo** | User reports Expo Go-side regression on the home / profile feeds after the PostGallery + TastingNoteCard changes landed (web preview renders correctly). Start with a device reload — `react-native-gesture-handler@2.28` + `GestureHandlerRootView` was installed mid-session so Expo Go may need a full rebuild before the new gesture stack activates. If the crash persists after rebuild, bisect by reverting `PostGallery` then `TastingNoteCard` landscape. Likely suspect: the `<Pressable>` wraps around gallery slots racing with the `SwipeToCommit` parent on native (web responder arbitration differs from iOS). |
| M2 | **Header + Footer buttons dead while PostModal is open (mobile)** | After tapping a post to open the modal, the MobileHeader (hamburger / bell / search) and MobileFooter (Home / Discover / Messages / Profile) stop responding to taps. Modal mounts as absolute inside the relative wrapper — suspect: either the modal host is extending beyond its `bottom: 0` bound on some layout path, or `elevation: 12` on native is swallowing outside taps. Also possible: the user meant the modal's OWN back button + ActionBar (inner header/footer) — clarify before fixing. Fix candidates: explicit `pointerEvents="box-none"` on `mobileHost`, tighter zIndex isolation, or mount the modal as a true `<RN Modal>` on mobile again and accept the paint-over-chrome cost. |
| M3 | **Message button on user profile (viewed as Nada) does nothing** | Signed in as Nada (roaster account), open another user's profile, tap "Message" in the bio — no-op. DM-thread creation path is probably gated on account type or missing its wiring after the §2.40.3 composer refactor. Cross-check `InterestedButton` flow as the analog — that one works for roasters viewing a café. |
| M4 | **Café profile missing a Message button entirely** | User profile has a Message CTA; café profile doesn't. Inconsistent. Either add a matching "Message this café" CTA on the café bio panel, or document why cafés are DM-exempt (likely: café ↔ roaster DMs route through the wholesale `InterestedButton` flow and cafés don't receive unsolicited DMs). If adding, mirror user-profile's button placement + endpoint. |
| M5 | **Add Image modal thumbnail — verify on native** | Fix applied (`resolveUploadUrl(previewUrl)` on the `<Image>` in `ImageUploadModal`); verified via code path but not yet on device. First test next session: trigger image upload via ComposePost's Add-Card flow on Expo Go, confirm thumbnail shows. |
| M6 | **Inner Pressables in cards blocked by outer Pressable wrapper on web** | PostGallery wraps `<TastingNoteCard>` in `<Pressable onPress={openModal}>` to make the whole card clickable. On web, the RN Web Pressable adds an onClick that fires via event bubbling — so the tasting-note card's inner cart button fires its own `openExternal(product_url)` AND the outer onPress opens PostModal. User-visible symptom: tapping cart opens both a new tab and the post modal. Fix: either drop the outer Pressable wrap for cards (tap elsewhere on the post to open modal — user still has the body text / avatar rows as tap surfaces), or add `e.stopPropagation()` inside the inner cart Pressable. Prefer dropping the wrap: simpler + matches the user's ask that card buttons "should be active". |
| M7 | **Tap-to-open doesn't fire on image thumbnails (native)** | On Expo Go, tapping an image in the 3-col strip doesn't open PostModal even though the `Pressable onPress` is wired. Likely cause: `SwipeToCommit`'s `Gesture.Pan()` on the outer PostCard row is eating the single-tap via the gesture arbiter. `activeOffsetX=[-10,10]` should let a zero-travel tap through, but the gesture may still be claiming the touch as "potential pan" on native. Fix candidates: add `.shouldCancelWhenOutside(true)` or restructure so the gallery's Pressable registers as a TapGesture that competes with the outer Pan (gesture-handler's `Gesture.Race` or `Gesture.Exclusive`). |
