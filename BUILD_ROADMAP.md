...
| 2.40.12 | **X-style draggable scroll scrubber on feed** | Right-edge thumb appears while scrolling, fades on idle. Drag to jump. Needs a custom `<ScrollScrubber>` component that reads the feed's scrollY + contentHeight + viewportHeight, maps the thumb position to scroll offset, and calls `scrollTo` on drag. Only on the Home feed for now; add to profile/roaster/cafe later if it proves useful. |
