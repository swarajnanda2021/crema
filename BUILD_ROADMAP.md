...
### 2.40 Mobile — sessions 1+2+3 (branch `feat/mobile-readiness`)
...
What still needs doing in session 4:

| # | Item | Notes |
|---|------|-------|
| 2.40.9 | **Scroll-aware chrome — sitewide wiring** | Home feed's ScrollView pipes `onScroll` → `onChromeScroll` today. Profile / user / roaster / cafe / browse (beans + roasters + cafés) / messages ScrollViews all still skip it. Add the one-liner per surface. |
| 2.40.10 | **MobileFooter full collapse residue** | Session 3's height-animated footer almost works but still leaves a thin strip visible at full-hide — likely the drop-shadow or the safe-area tail leaking past the outer clip. Inspect: dev server mobile viewport, scroll down fast, check the bottom edge for a 2–6 px cream stripe. Candidate fix: animate `shadowOpacity` → 0 in parallel, or lift the whole thing (SafeAreaView fill included) inside the height-clipping wrapper. |
| 2.40.11 | **Swipe-left-to-like / swipe-right-to-comment on feed rows** | Per user spec, match X/Instagram: the action bar disappears from the feed card entirely; swipe-left reveals a Crema-pink heart disc that fills as the swipe completes (triggers like); swipe-right reveals a Crema-pink comment disc that fills as the swipe completes (opens the PostModal in comment mode). The ActionBar stays inside PostModal for explicit taps. Needs a new gesture primitive — conceptually extend `SwipeableRow` to emit a "completion" event as the swipe crosses a threshold, with the revealed icon animating its inner fill from transparent to `accent`. |
| 2.33 | **CoffeeCard landscape variant — SITEWIDE** | Per Figma 109:9154 — 360×251 row, 50/50 image/info split. The user wants this applied everywhere a CoffeeCard renders (feed, shelf carousel, tasting-note rows, popularity modal, discover grid). One component change, many call-site proofs. |
